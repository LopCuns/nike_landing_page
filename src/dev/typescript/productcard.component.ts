...
class ProductCardComponent extends HTMLElement{
  ...
  connectedCallback() {
    ...
    const colors : string[] = this.getAttribute('data-colors')?.split('|') as string[]
    if (colors[0]) this.style.setProperty('--color1',colors[0])
    if (colors[1]) this.style.setProperty('--color2',colors[1])
    if (colors[2]) this.style.setProperty('--color3',colors[2])
